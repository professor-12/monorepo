                              ...
                              <div className='grid grid-cols-3 max-md:grid-cols-2 max-sm:grid-cols-2'>
                                    {
                                          data?.data && data?.data?.map((item) => {
                                                // console.log(item?.reactions, user?.uid)
                                                ...
                                                return (
                                                      <div key={item.id} className='m-2 hover:shadow border border-border  rounded-md overflow-hidden'>
                                                            <div style={{ backgroundImage: `url('${item?.thumbnail}')` }} className="w-full h-[9rem] bg-cover bg-[6px] bg-[url('/assets_.webp')] relative">
                                                                  <div onClick={() => { navigateToProfile(item?.author?.profile?.displayName) }} style={{ backgroundImage: `url('${item?.author?.profile?.picture}')` }} className={`absolute top-[calc(100%-30px)] left-4 w-12 h-12 bg-cover bg-center bg-[url('/assets_.webp')] ring-white ring-4 rounded-xl`}></div>
                                                            </div>
                                                            <div className='p-3 pt-10 space-y-1'>
                                                                  <div className='h-[5.8rem]'>
                                                                        <h1 className='font-medium text-sm'>{item?.author?.profile?.displayName}</h1>
